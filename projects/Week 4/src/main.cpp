...
int main() {
	...
	VertexArrayObject::Sptr vao_2 = VertexArrayObject::Create();
	vao_2->AddVertexBuffer(posVbo2, {
		BufferAttribute(0, 3, AttributeType::Float, 0, NULL)
		});
	vao_2->AddVertexBuffer(color_vbo2, {
		{ 1, 3, AttributeType::Float, 0, NULL }
	});
	...
	while (!glfwWindowShouldClose(window)) {
		...
		transform_2 = glm::translate(glm::mat4(1.0f), glm::vec3(glm::sin(static_cast<float>(thisFrame)), -1.0f, 0.0f));
		...
		shader->SetUniformMatrix("u_ModelViewProjection", camera->GetViewProjection()* transform_2);
		vao_2->Bind();
